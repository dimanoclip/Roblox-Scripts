...
if volleyball_ids[tostring(game.PlaceId)] then
    ...
    local lastserver, antishiftlock, superhitbox
        ...
        if child == getball() then
            child:WaitForChild("HitBox").Touched:Connect(function(part)
                if part.Parent == lp.Character then
                    ...
                    elseif is_midair() and RP:GetAttribute("LastHitTeam") == tostring(lp.Team) and (attack[RP:GetAttribute("LastHitType")] or recieves[RP:GetAttribute("LastHitType")]) then
                        vinp.CenterMouseClick()
                        -- if child.PrimaryPart.CFrame.Position.Y >= lp.Character.PrimaryPart.CFrame.Position.Y then
                        -- end
                    end
    ...
    task.spawn(function()
        if hookmetamethod then
            ...
            hook_handler = hookmetamethod(game, "__namecall", function(self, ...)
                ...
                elseif self == interact and (args[1]["Move"] == "Dive") then
                    superhitbox = false
                    return hook_handler(self, ...)
                elseif self == hitbox and (args[1]["Move"] == "Dive") then
                    superhitbox = false
                    return hook_handler(self, ...)
    ...
    task.spawn(function()
        while game:GetService("RunService").RenderStepped:Wait() do
            if lp.Character:FindFirstChild("Humanoid") then
                update_hitbox((superhitbox and 4 or is_midair() and 2 or 1.7)+(math.floor((getping()-50)/50)))
            end
            ...
            lp:SetAttribute("Multiplier_Speed", 1)
            lp:SetAttribute("Multiplier_SetPower", 1)
            lp:SetAttribute("Multiplier_ServePower", 1)
        end
    end)
    task.spawn(function()
        while game:GetService("RunService").RenderStepped:Wait() do
